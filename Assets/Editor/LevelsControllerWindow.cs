...
public class LevelsControllerWindow : EditorWindow
{
    Vector2 scrollPos;
    ...
    [MenuItem("Window/Levels Controller")]
    public static void ShowWindow()
    {
        GetWindow<LevelsControllerWindow>("Levels Controller Window");
    }
    ...
    private void OnGUI()
    {
        ...
        if (GUILayout.Button("CLEAR Current Cover"))
        {
            if (currentGenerator)
            {
        ...
        GUILayout.Space(20f);
        GUILayout.Label("Current Room", EditorStyles.boldLabel);

        currentRoomEnemySystem =
            (RoomEnemySystem) EditorGUILayout.ObjectField(currentRoomEnemySystem, typeof(RoomEnemySystem), true);
        GUILayout.Label("", EditorStyles.boldLabel);
        //
        //
        // int bodiesFound = 0;
        if (GUILayout.Button("Save Current Room"))
        {
            Debug.Log($"Saving {currentRoomEnemySystem.transform.parent}");
            if (currentRoomEnemySystem)
            {
                RoomEnemySaveLoader.SaveData(currentRoomEnemySystem);
            }
            else
            {
                Debug.LogWarning("Missing Current Room");
            }

            EditorSceneManager.MarkSceneDirty(EditorSceneManager.GetActiveScene());
        }

        if (GUILayout.Button("Load Current Room"))
        {
            Debug.Log($"Loading {currentRoomEnemySystem.transform.parent}");

            if (currentRoomEnemySystem)
            {
                RoomEnemySaveLoader.LoadData(currentRoomEnemySystem);
            }
            else
            {
                Debug.LogWarning("Missing Current Room");
        ...
        if (GUILayout.Button("Save All Room"))
        {
            foreach (RoomEnemySystem roomEnemySystem in FindObjectsOfType<RoomEnemySystem>())
            {
                Debug.Log($"Saving {roomEnemySystem.transform.parent}");
                if (roomEnemySystem)
                {
                    RoomEnemySaveLoader.SaveData(roomEnemySystem);
                }
                else
                {
                    Debug.LogWarning("Missing Current Room");
        ...
        if (GUILayout.Button("Load All Room"))
        {
            foreach (RoomEnemySystem roomEnemySystem in FindObjectsOfType<RoomEnemySystem>())
            {
                Debug.Log($"Loading {roomEnemySystem.transform.parent}");
                if (roomEnemySystem)
                {
                    RoomEnemySaveLoader.LoadData(roomEnemySystem);
                }
                else
                {
                    Debug.LogWarning("Missing Current Room");
...
public class RoomEnemySaveLoader
{
    static string path = "Assets/Resources/Levels/LevelEnemySpawnData/";
    private static string fileType = "_LevelSpawn.JSON";

    public static void SaveData(RoomEnemySystem roomEnemySystem)
    {
        FileLoader.SaveToFile(path, GenerateFileName(roomEnemySystem), roomEnemySystem.GetData());
        UnityEditor.AssetDatabase.Refresh();
    }

    private static string GenerateFileName(RoomEnemySystem roomEnemySystem)
    {
        return roomEnemySystem.transform.parent.name + fileType;
    }

    public static void LoadData(RoomEnemySystem roomEnemySystem)
    {
        roomEnemySystem.LoadData(
            FileLoader.LoadFromFile<RoomEnemySystemData>(path + GenerateFileName(roomEnemySystem)));
    }
}

public class LevelsControllerWindowInspector : Editor
{
