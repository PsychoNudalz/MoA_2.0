...
public class GunDamageScript : DamageScript
{
    ...
    void CorrectRecoil()
    {
        ...
        if (currentRecoil.y > .01f)
        {
            currentRecoil.y -= recoil.y * Time.deltaTime / timeToRecenter;
        }
        else if (currentRecoil.y < -.01f)
        {
            currentRecoil.y += recoil.y * Time.deltaTime / timeToRecenter;
        }
        else
    ...
    {
        RaycastHit hit;
        randomFireDir = new Vector2(Random.Range(0, recoilPattern_X.Evaluate(currentRecoilTime) * recoil.x), Random.Range(-180f, 180f));
